    ...
    def parse(self) -> List[str]:
        ...
        try:
            with sync_playwright() as playwright:
                ...
                # Очищаем куки для нового контекста
                context.clear_cookies()
                self.logger.info(
                    'Куки очищены для нового контекста Playwright'
                    )
                ...
                while True:
                    ...
                    # Очищаем куки после перехода на страницу
                    context.clear_cookies()
...
class TdpplSpider(BaseCompetitorSpider):
    ...
    # Настройки специальных экспортеров для Playwright и поддержки cookies
    custom_settings = {
        'ITEM_PIPELINES': {
        ...
        'DOWNLOADER_MIDDLEWARES': {
            ...
            'competitors_parser.middlewares.ErrorHandlerMiddleware': 750,
        }
    }

    def start_requests(self):
        """Переопределяем метод start_requests для очистки кук перед началом парсинга."""
        self.logger.info(
            'Начало работы парсера tdppl. Удаляем куки перед запросами.'
            )

        # Создаем начальный запрос с очисткой кук
        for url in self.start_urls:
            yield Request(
                url=url,
                callback=self.parse,
                cookies={},  # Пустой словарь для очистки существующих кук
                meta={'cookiejar': 1},  # Уникальный ID для cookie jar
                dont_filter=True
            )
    ...
    def parse(self, response: Response) -> Iterator[Request]:
        ...
        for category in categories:
            ...
            yield Request(
                ...
                cookies={},  # Очищаем куки для каждого запроса
                meta={'cookiejar': 1}  # Используем тот же ID для cookie jar
            )
